...
function cloneAircraft(ac: AircraftState): AircraftState {
  return {
    ...
    portTailplaneRootHits: ac.portTailplaneRootHits ?? 0,
    starboardTailplaneRootHits: ac.starboardTailplaneRootHits ?? 0,
    windowHeatHits: ac.windowHeatHits ?? 0,
...
export function runMission(
  ...
  if (!planeDestroyed && currentZone <= 1 && direction === 'inbound') {
    ...
    const landMod = calculateLandModifier({
      ...
      tailWheelInop: ac.tailWheelInop,
      controlDamage: ac.controlDamage,
      portFlapInop: ac.portFlapInop,
      starboardFlapInop: ac.starboardFlapInop,
      portAileronInop: ac.portAileronInop,
      starboardAileronInop: ac.starboardAileronInop,
      portElevatorInop: ac.portElevatorInop,
      starboardElevatorInop: ac.starboardElevatorInop,
      bipDamage,
